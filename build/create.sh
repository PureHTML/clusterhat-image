...
if [ $# -ne 1 ]; then
  ...
  echo " Where version is 2023-05-03 2023-05-03-raspios-bullseye-armhf-lite.zip"
...
# Check for Raspbian bullseye
if [ -f "$SOURCE/$VER-raspbian-bullseye-lite.img" ]; then
  SOURCES[$CNT]="$VER-raspbian-bullseye-lite.img|$VER-$REV-bullseye-ClusterCTRL-armhf-lite|LITE|bullseye"
  let CNT=$CNT+1
fi
if [ -f "$SOURCE/$VER-raspbian-bullseye.img" ]; then
  SOURCES[$CNT]="$VER-raspbian-bullseye.img|$VER-$REV-bullseye-ClusterCTRL-armhf-std|STD|bullseye"
  let CNT=$CNT+1
fi
if [ -f "$SOURCE/$VER-raspbian-bullseye-full.img" ]; then
  SOURCES[$CNT]="$VER-raspbian-bullseye-full.img|$VER-$REV-bullseye-ClusterCTRL-armhf-full|FULL|bullseye"
  let CNT=$CNT+1
fi

# Check for Raspberry Pi OS (old naming scheme)
if [ -f "$SOURCE/$VER-raspios-bullseye-lite-armhf.img" ]; then
  SOURCES[$CNT]="$VER-raspios-bullseye-lite-armhf.img|$VER-$REV-bullseye-ClusterCTRL-armhf-lite|LITE|RASPIOS32bullseye"
  let CNT=$CNT+1
fi
...
# let CNT=$CNT+1
#fi
if [ -f "$SOURCE/$VER-raspios-bullseye-full-armhf.img" ]; then
  SOURCES[$CNT]="$VER-raspios-bullseye-full-armhf.img|$VER-$REV-bullseye-ClusterCTRL-armhf-full|FULL|RASPIOS32bullseye"
  let CNT=$CNT+1
fi
...
EOF
    ...
    if [ "$SERIALAUTOLOGIN" = "1" ]; then
      if [ $RELEASE = "bullseye" -o $RELEASE = "RASPIOS32bullseye" -o $RELEASE = "RASPIOS64bullseye" -o $RELEASE = "RASPIOS32BULLSEYE" -o $RELEASE = "RASPIOS64BULLSEYE" ]; then
...
EOF
  ...
  if [ ${!USBBOOT} = "1" ] && [ -f "$DEST/$DESTFILENAME-CBRIDGE.img" ] && [ ! -f "$DEST/$DESTFILENAME-usbboot.tar.xz" ] && [ ! -f "$DEST/$DESTFILENAME-usbboot.tar" ]; then
    ...
    if [ $RELEASE = "RASPIOS64bullseye" -o $RELEASE = "RASPIOS64BULLSEYE" ]; then
    ...
    if [ "$SERIALAUTOLOGIN" = "1" ]; then
      if [ $RELEASE = "bullseye" -o $RELEASE = "RASPIOS32bullseye" -o $RELEASE = "RASPIOS64bullseye" ]; then
...
EOF
    ...
    # 3A+/CM3/CM3+
    for V in $( (
      cd $MNT2/root/lib/modules/
      ls | grep v7 | grep -v v7l | sort -V | tail -n1
    )); do
